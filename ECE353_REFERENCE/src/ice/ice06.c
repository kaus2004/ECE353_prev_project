...
void task_print_directions(void *arg)
{
    ...
    //waiting for joystick_posiition_t 
    xQueueReceive(Queue_Joystick, &position, portMAX_DELAY);
    printf("Joystick Position Changed: %s\n\r", Joystick_Pos_Strings[position]); //this is for Position names of joystick
                                                                                 //Document is lowkey tweaking
...
void app_main(void)
{
    ...
    xTaskCreate(task_print_directions, "Print Direction Task", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
