...
cyhal_i2c_t * i2c_init(cyhal_gpio_t sda, cyhal_gpio_t scl)
{
	...
	rslt = cyhal_i2c_init(i2c_obj, sda, scl, NULL);

	if (rslt != CY_RSLT_SUCCESS)
	{
		printf("Master I2C Initialization failed\r\n");
		return NULL;
	}
	...
	if (rslt != CY_RSLT_SUCCESS)
	{
		printf("Master I2C Configuration failed\r\n");
		return NULL;
	}
...
//data comes as 2 bytes
{
	...
	*value = (rx_data[0] << 8) | rx_data[1]; // Combine the two bytes into a 16-bit value
