...
__STATIC_INLINE void lcd_write_cmd_u8(uint8_t DL)
{
  ...
  //set the DCX pin high to indicate that we are done writing a command
  PORT_LCD_DCX->OUT_SET= MASK_LCD_DCX;

  //set the CSX pin high to indicate that we are done writing to the LCD
...
*  Nothing
*******************************************************************************/
//came from the manufacturer of the LCD
...
cy_rslt_t lcd_initialize(void)
{
  ...
  {
    printf("LCD GPIO configuration failed with error: %ld\n", result);
    return result;
  }
